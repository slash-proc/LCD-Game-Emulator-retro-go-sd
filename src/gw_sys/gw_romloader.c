...
// Internal buffer for hardware JPEG decoder (device only — RAM_EMU BSS)
...
//ROM in RAM_EMU BSS (not ITCM, not AHB — SD cores have a tight AHB heap;
// DTCM is for small hot state via dtc_*; ITCM is code-only in this core).
...
#ifdef HOST_BUILD
      /* Host has no STM32 JPEG peripheral — leave white background. */
      printf("HOST_BUILD: skipping hardware JPEG decode\n");
      gw_background = NULL;
#else
